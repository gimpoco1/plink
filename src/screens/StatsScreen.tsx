    ...
    const topPlayers = ranked
      ...
      .sort(
        (a, b) =>
          ...
          b.stats.currentWinStreak - a.stats.currentWinStreak ||
        ...
        <>
          <div className="statsHeroGrid">
            <SummaryCard
              ...
              copy="Finished sessions tracked locally."
            />
            <SummaryCard
              label="In progress"
              value={overview.activeGames}
              copy="Games still live right now."
            />
            <SummaryCard
              label="Top player"
              value={
                overview.topPlayer
                  ? getProfileDisplayName(overview.topPlayer.profile)
                  : "—"
              }
              copy={
                overview.topPlayer
                  ? `${overview.topPlayer.stats.wins} wins`
                  : "No winners yet"
              }
            />
            <SummaryCard
              ...
              value={
                overview.hottestStreak?.stats.currentWinStreak
                  ? `${overview.hottestStreak.stats.currentWinStreak}x`
                  : "—"
              }
              copy={
                overview.hottestStreak?.stats.currentWinStreak
                  ? getProfileDisplayName(overview.hottestStreak.profile)
                  : "No active streaks"
              }
            />
          </div>
          <div className="statsPanels">
            <section className="statsPanel">
              <PanelHeader
                title="Top players"
                count={overview.topPlayers.length}
              />
              {overview.topPlayers.length ? (
                <div className="statsList">
                  {overview.topPlayers.map(({ profile, stats }, index) => (
            ...
            <section className="statsPanel">
              <PanelHeader
                title="Popular games"
                count={overview.popularGames.length}
              />
              {overview.popularGames.length ? (
                <div className="statsList">
                  {overview.popularGames.map((game, index) => (
                    <div key={game.name} className="statsRow">
                      ...
                      <span className="statsRow__value">{game.sessions}</span>
                    </div>
                  ))}
                </div>
              ) : (
...
function SummaryCard({
  ...
  return (
    <div className={`statsHeroCard${accent ? " statsHeroCard--accent" : ""}`}>
      <span className="statsHeroCard__label">{label}</span>
      <strong>{value}</strong>
      <p>{copy}</p>
    </div>
  );
