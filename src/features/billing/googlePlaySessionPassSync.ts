...
async function readInvokeError(error: unknown) {
  if (error instanceof FunctionsHttpError) {
    try {
      ...
      return "Google Play could not verify this Session Pass.";
    }
  }
  if (
  ...
  return error instanceof Error
    ? error.message
    : "Google Play could not verify this Session Pass.";
}

export async function syncGooglePlaySessionPass(purchaseToken: string) {
