create or replace function public.update_shared_game_settings_v5(
  p_game_id text, p_name text, p_score_direction text, p_starting_score integer,
  p_target_score integer, p_win_condition text, p_win_by_two boolean,
  p_manual_end_only boolean, p_timer_enabled boolean, p_tools_enabled boolean,
  p_quick_score_value_1 integer, p_quick_score_value_2 integer,
  p_timer_mode text, p_timer_seconds integer, p_collaborators_can_manage boolean
)
...
declare
  ...
  v_runner_up integer;
begin
  if auth.uid() is null then raise exception 'Sign in to update this game.' using errcode = '42501'; end if;
  if p_starting_score is null or p_target_score is null or p_win_by_two is null
    or p_manual_end_only is null or p_timer_enabled is null or p_tools_enabled is null
  ...
  if p_quick_score_value_1 not between 1 and 999999 or p_quick_score_value_2 not between 1 and 999999
  ...
  if not found or not (v_game.user_id = auth.uid() or exists (
  ...
  update public.games set
    ...
    collaborators_can_manage = case when v_game.user_id = auth.uid() then p_collaborators_can_manage else v_game.collaborators_can_manage end,
